...
SLU_Hitting_new <- SLU_Hitting %>%
  ...
  separate(col = Player,
         ...
         sep = ",") %>%
  select(c(-3)) %>%
  mutate(image = sample("https://www.pngfind.com/pngs/m/293-2932598_st-lawrence-university-logo-png-transparent-png.png", size = 1, replace = TRUE))
...
Skidmore_Hitting_new <- Skidmore_Hitting %>%
  ...
  separate(col = Player,
         ...
         sep = ",") %>%
  select(c(-3)) %>%
  mutate(image = sample("https://www.pinclipart.com/picdir/middle/350-3501802_skidmore-college-skidmore-college-logo-clipart.png", size = 1, replace = TRUE))
...
aes(x = reorder(Player, .data[[input$StatChoiceH]]),y=.data[[input$StatChoiceH]], color = School)) +
  ...
  geom_segment(aes(x = Player, xend = Player, y = 0, yend = .data[[input$StatChoiceH]])) +
  coord_flip() +
